  ...
  describe 'As a Visitor' do
    ...
    describe 'I can manipulate my cart' do
      ...
      it "i can apply a discount" do
        ...
        fill_in :coupon_name, with: 'marma10'

        click_button 'Apply Coupon'

        expect(current_path).to eq('/cart')
        expect(page).to have_content("#{@marma_10.name} has been applied.")
        expect(page).to have_content("Discounted Total: $30.00")

        fill_in :coupon_name, with: 'marma'

        click_button 'Apply Coupon'

        expect(current_path).to eq('/cart')
        expect(page).to have_content("marma is not a valid coupon.")
      end
    end
  end
end

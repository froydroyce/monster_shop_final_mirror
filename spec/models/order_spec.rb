...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @order_1 = @user.orders.create!(address_id: @user_address.id, status: "packaged")
      @order_2 = @user.orders.create!(address_id: @user_address.id, status: "pending")
  ...
  describe 'class methods' do
    before :each do
      @user = User.create!(name: 'Megan', email: 'megan_1@example.com', password: 'securepassword')
      @user_address = @user.addresses.create!(address_name: "Home", address: "1234 Main st", city: "Denver", state: "CO", zip: 80229)
      @order_1 = @user.orders.create!(address_id: @user_address.id, status: 1)
      @order_2 = @user.orders.create!(address_id: @user_address.id, status: 0)
      @order_3 = @user.orders.create!(address_id: @user_address.id, status: 3)
